...
if (mcmc_engine == "stan") {
  mod_fit <- as_cmdstan_fit(list.files(str_glue("{mcmc_engine}/draws/{model_name}"), 
                                     full.names = TRUE))
} else { # == "jags"
  mod_fit <- read_rds(str_glue("{mcmc_engine}/draws/{model_name}.rds"))
}

mod_summary <- read_csv(str_glue("{mcmc_engine}/summary/{model_name}_summary.csv"))
```
