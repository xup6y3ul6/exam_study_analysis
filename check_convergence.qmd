...
if (params$mcmc_engine == "stan") {
  mod_fit <- as_cmdstan_fit(list.files(str_glue("{params$mcmc_engine}/draws/{model_name}"), 
                                     full.names = TRUE))
} else { # == "jags"
  mod_fit <- read_rds(str_glue("{params$mcmc_engine}/draws/{model_name}.rds"))
}

mod_summary <- read_csv(str_glue("{params$mcmc_engine}/summary/{model_name}_summary.csv"))
```
...
if (params$mcmc_engine == "stan") {
  ...
  mod_draws <- as_draws_df(mod_fit$samples) |> thin_draws(5)
}
...
mcmc_trace(mod_draws, pars = vars(starts_with("sigma_", ignore.case = FALSE)))
```
